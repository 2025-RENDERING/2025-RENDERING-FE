  ...
  return (
    <header className={`fixed inset-x-0 top-0 z-50 flex h-16 items-center px-[24px] ${isExhibition ? "bg-blue-normal/5" : "bg-grey-normal"} ${textColorClass}`}>
      <button
        ...
        disabled={isRoot}
      >
        <BackIcon className="h-[14px] w-[7px]" stroke={isExhibition ? "#FFFFFF" : "#464443"} />
      </button>
      <h1 className={`flex-1 text-center text-l-medium ${textColorClass}`}>{title}</h1>
      <button type="button" aria-label="메뉴 열기" className={`flex h-10 w-10 items-center justify-center rounded-full opacity-80 ${textColorClass}`}>
        <HamburgerIcon className="h-[14px] w-[20px]" stroke={isExhibition ? "#FFFFFF" : "#464443"} />
      </button>
    </header>
  );
};
